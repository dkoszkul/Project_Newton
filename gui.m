...
H=hessian(y, [x1 x2 x3 x4 x5 x6]); %liczy hessian symbolicznie
if(H==0)
    text_to_check = 'Hessian cannot be inverted. Check your function.';
    ...
    if(strcmp(checkSubmatrices(evaluated_fx(H,X)),'local maximum'))
        text_to_check = sprintf('%s. %s',checkSubmatrices(evaluated_fx(H,X)),'Computation stopped');
        set(handles.iteracje_Edit,'String',text_to_check);
        set(handles.pointType,'String',text_to_check);
        set(handles.result_Edit,'String',text_to_check);
    else
        ...
        text_to_check = 'local minimum or maximum does not exist';
